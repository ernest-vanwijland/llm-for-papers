...
The strategy $\sigma$ uses memory of size at most  $\abs{Q}\cdot\abs{E}$ since $W \subseteq Q$. 
	...
	As phase $(2)$ of the strategy $\sigma$ is eventually executed forever with probability~$1$, let $e$
	be the corresponding environment (i.e., such that $\sigma$ plays according to $\sigma_e$)
	and let $D \neq \{\winabsorb\}$ be the reached end-component of $M[e]$ (the other case where $\winabsorb$
	is reached is trivial). 
	If some transition of $D$ is not present in $f$,
	then it must be a revealing transition in $e$, thus leading in $M[e]$ to $\winabsorb$ outside $D$,
	which is impossible since $D$ is an end-component in $M[e]$.
	Hence all transitions of $D$ are present in all environments.

	We show that $\sigma$ is almost-sure winning in $f$. 
	The result is immediate if $D$ is an end-component of $M[f]$ (in particular if $f = e$). 
	If $D$ is not an end-component of $M[f]$, then in $M[f]$ the strategy would leave $D$ and 
	reach $\winabsorb$, thus $\sigma$ is almost-sure winning as well in that case.
\end{proof}

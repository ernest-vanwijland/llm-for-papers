...
\end{lemma}
\begin{proof}
    Let $U=N(v)$ and $W=V\smallsetminus(N(v)\cup\{v\})$.

    If there is $y\in U\cup W$ that has two distinct neighbors $u$ and $u'$ in $U$, then $v,u,y,u'$ form a 4-cycle. Suppose on the contrary that every $y\in U\cup W$ has at most one neighbor in $U$. Then,
    \[
    \begin{split}
        \sum_{u\in U}\deg (u) &= \deg(v)+\sum_{u\in U} \deg_U(u)+\sum_{w\in W}\deg_U(w)\\
        &\leq |U|+|U|+|W|\\
        &\leq 2\times|U\cup W|\leq 2n.
    \end{split}
    \]
\end{proof}
...
\begin{figure}[htb]
    \centerline{\includegraphics[scale=0.5]{figures/paths-PPP.pdf}}
